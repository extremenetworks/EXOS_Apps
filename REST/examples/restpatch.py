...
#
#usage: restpatch [-h] [-i IPADDRESS] [-u USERNAME] [-p PASSWORD] [-d]
#
#optional arguments:
...
class RestPatch(Restconf):
    def __init__(self, ipaddress, username, password, debug=False):
        super(RestPatch, self).__init__(ipaddress, username, password, debug=debug)
    ...
    def __call__(self):
        ...
        while True:
            ...
            patch_url = url_prefix + input_url
            patch_data = raw_input('Enter the body of the request: ')
            patch_json = json.loads(patch_data)
            try:
                # send the command to the EXOS switch over HTTP.
                # the object will do the proper encoding
                response = self.patch(patch_url, patch_json)
...
rest_obj = RestPatch(args.ipaddress, args.username, args.password)
try:
    rest_obj()
